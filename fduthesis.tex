\documentclass[oneside,type=master]{fduthesis}
...
人机交互是一个研究技术怎么使用和设计的学科，其中一个重要的共识是上下文与场景对我们的日常行为和技术使用起到关键性作用：当场景发生变化之后，对于技术怎么使用和怎么设计存在关键性影响。
...
针对日常健康场景下面诊系统如何设计与实现这一问题，主要工作如下:

1. 将技术探针方法应用到日常健康场景下面诊技术的用户研究中。
日常健康场景与诊所的标准环境相比，场景更加复杂，用户真实想法难以获取。
与相对可控的实验和专业场景不同，日常场景下做用户研究难点在于日常场景复杂多样，并且包括很多家庭空间中的私密空间，因此很难深入到这些场所用实际跟踪和观察的方式去收集用户使用技术的数据。
技术探针是人机交互领域中一种用户参与的设计方法，该方法能够把用户带到真实的场景中来收集用户相关信息，并且有一套激发用户参与设计的方法论。
为了更好地理解日常健康场景下面诊技术的使用问题，本文首次尝试将技术探针应用到日常健康场景下对面诊技术进行了用户研究。

2. 发现日常健康场景下面诊技术存在的使用问题并提出设计策略。
...
通过基于有向无环图的模型管理，解决面诊技术更新迭代的问题。

4. 实现了面向日常健康的面诊系统设计方案并进行验证。
基于日常健康场景中的面诊系统设计方案，本文按照设计方案对系统进行了实现。最后通过用户体验评估和系统性能分析两个实验，分别进行了验证。
