...
1.利用技术探针方法揭示面诊技术应用在日常健康场景下存在的交互设计挑战。
技术探针是交互领域中一种用户参与的设计方法，该方法能够把用户带到真实的场景中来收集用户相关信息。本文基于诊所环境的云中医智能镜，开发出了云中医应用作为技术探针，招募用户在其日常生活中进行为期3周的实地试用，并对他们使用前后的体验进行深度用户访谈。定性分析转录成文字后的访谈资料，发现了面诊技术在日常健康场景下会遇到持续使用、实用性、敏感性、情绪与信赖相关的交互设计挑战，以及用户设备多样性带来的稳定性问题等方面的问题。

2.针对之前发现的面诊技术应用于日常健康场景存在的设计挑战，提出了相应的系统设计方案。
在交互设计层面，初步提出了对应的设计策略：增加系统可解释性、提高日常可用性，以增强系统实用性和用户信赖感；
在系统设计层面，提出通过跨平台、分布式模型、诊断任务分解与基于有向无环图管理的方式解决用户设备多样性的问题，提高了面诊系统稳定性，方便应对大规模的用户请求，并降低模型迭代带来的成本，为当前众多类别的面诊系统设计提供了统一的解决方案。

3.实现了面向日常健康的面诊系统设计方案并进行验证。
通过跨平台设计、面诊模型管理、诊断任务分解等模块，解决用户设备的差异性带来的稳定性和性能的问题；通过交互设计，增加系统可解释性和日常可用性，一定程度上解决长期使用、实用性、敏感性、情绪与依赖等问题。最后通过系统性能分析和用户体验评估两个实验，分别进行了验证。
