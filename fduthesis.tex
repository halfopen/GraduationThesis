...
1.利用技术探针探索日常场景下面诊应用的用户研究。
技术探针是交互领域中一种用户参与的设计方法，把用户带到真实的场景中来收集用户相关信息。本文使用诊所环境下的一款面诊应用（云中医）作为技术探针，通过对参与者在日常场景下使用云中医的体验情况进行深度访谈，分析转录成文字后的访谈资料，发现了面诊技术在日常场景下会遇到长期使用、实用性、敏感性、情绪与信赖以及用户设备多样性带来的稳定性问题等方面的问题。

2.提出日常健康场景下面诊系统的系统设计。
为了解决在用户研究中发现的诸多问题，本文提出了面向日常健康场景的面诊系统设计。
在交互设计层面，初步提出了对应的设计思路：增加系统可解释性、提高日常可用性；
在系统设计层面，提出通过跨平台、分布式模型、诊断任务分解与基于有向无环图管理的方式解决用户设备多样性的问题，提高了面诊系统稳定性，方便应对大规模的用户请求，并降低模型迭代带来的成本。
为当前众多类别的面诊系统设计提供了统一的解决方案。

3.实现了面向日常健康场景的面诊系统并进行验证。
通过跨平台设计、面诊模型管理、诊断任务分解等模块，解决用户设备的差异性带来的稳定性和性能的问题；通过交互设计，增加系统可解释性和日常可用性，一定程度上解决长期使用、实用性、敏感性、情绪与依赖等问题。最后通过系统性能分析和用户评价两个实验，分别进行了验证。
