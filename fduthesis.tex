...
面诊技术，即通过读取人的面部信息获取相关特征进而大致判断人的健康状况等技术。
本文主要关注基于面诊技术的面诊系统在日常健康场景下如何设计与实现的问题。
目前已有的面诊系统主要是面向诊所医院等专业环境，面向日常健康场景下的面诊系统如何设计还存在诸多挑战。
...
技术探针是交互领域中一种用户参与的设计方法，该方法能够把用户带到真实的场景中来收集用户相关信息。本文使用基于诊所环境开发出来的一款面诊应用的移动端版本（云中医）作为技术探针，招募用户在其日常生活中进行为期3周的实地试用，并对他们使用前后的体验进行深度用户访谈。定性分析转录成文字后的访谈资料，发现了面诊技术在日常健康场景下会遇到持续使用、实用性、敏感性、情绪与信赖相关的交互设计挑战，以及用户设备多样性带来的稳定性问题等方面的问题。
