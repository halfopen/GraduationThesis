...
\begin{document}
% \includepdf[pages={1,1}]{mangshenfengmian.pdf}  % 嵌入封面
% \raggedbottom
...
将面诊技术从专业的诊所环境应用到日常场景，能在一定程度上方便人们进行日常健康管理，缓解当前整体公共医疗资源不足的压力，帮助人们更加健康地生活。

针对日常场景下面诊系统如何设计与实现这一问题，主要工作如下:

1. 将技术探针方法应用到日常场景下面诊技术的用户研究中。
与相对可控的实验和专业场景不同，日常场景下做用户研究的难点在于日常场景复杂多样，并且包括很多家庭空间中的私密空间，因此很难深入到这些场所用实际跟踪和观察的方式去收集用户使用技术的数据。
技术探针是人机交互领域中一种用户参与的设计方法，该方法能够把用户带到真实的场景中来收集用户相关信息，并且有一套激发用户参与设计的方法论。
为了更好地理解日常场景下面诊技术的使用问题，本文首次尝试将技术探针应用到日常场景下对面诊技术进行了用户研究。

2. 发现日常场景下面诊技术存在的使用问题并提出设计策略。
(1)通过分析技术探针获取的数据，发现了面诊技术在日常场景下会遇到持续使用、实用性、敏感性、情绪与信赖相关的交互设计问题，以及用户设备多样性带来的稳定性问题等方面的问题。
(2)针对发现的问题，本文提出了对应的设计策略：增加系统可解释性、提高日常可用性、支持可持续使用等方面以指导未来的面诊系统设计。

3. 针对之前发现的面诊技术应用于日常场景存在的设计问题，根据交互策略提出了相应的系统设计方案。
(1)在交互设计层面，通过优化面诊交互流程、对系统进行解释、对结果和诊断规则的合理可视化展示等方式，解决日常场景下系统实用性和用户情绪与信赖的问题。
...
基于日常场景中的面诊系统设计方案，本文对系统进行了实现。最后通过用户体验评估和系统性能分析等实验，分别进行了验证。
