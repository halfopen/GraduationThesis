...
\begin{figure}[ht]
    ...
    \label{fig:server}
\end{figure}
目前用到的模型，基于opencv和svm，实现了图片特征的提取和分类。在每个模型的基础上，最后由一个诊断规则根据每个模型的输出，得到最终的诊断结果。
svm分类器的训练数据来自各个医院的真实数据。
...
由于模型中的算法来自云中医，因此算法的具体实现不多做介绍，接下来只介绍模型的输入输出的功能，模型的计算能力如表\ref{tab:face-feature}, 表\ref{tab:tongue-feature}, 表\ref{tab:diag-feature}所示。
\subsection{脸部特征提取模型}
